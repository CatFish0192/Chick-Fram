...
def logo(status="start"):
    ...
    endlogo = {
    "1":r"   ______                ______",
    "2":r"  / ____/___  ____  ____/ / __ )__  _____",
    "3":r" / / __/ __ \/ __ \/ __  / __  / / / / _ \\",
    "4":r"/ /_/ / /_/ / /_/ / /_/ / /_/ / /_/ /  __/",
    "5":r"\____/\____/\____/\__,_/_____/\__, /\___/",
    "6":r"                             /____/"
    }
...
https://gitcode.com/bxl24563/XJZY
    ...
    if not o.path.exists(dbpath):
        ...
        input("对了,这些给你(5个金币,10个饲料)好了,我不打扰了,希望我再回来的时候,你能打造出一片最大的庄园!")
        input("(老人离开了这里······)")
        t.sleep(1)
        clear()
        if not neco:
        ...
        if xjzydbresult != None: #数据库为空
            ...
            if json_Gold != Gold or json_Stealth_potions != Stealth_potions or json_How_much_feed_the_chicks_ate_in_total != How_much_feed_the_chicks_ate_in_total or json_Accelerator_card != Accelerator_card or json_How_much_corn_is_there_in_total != How_much_corn_is_there_in_total or json_How_many_corn_seeds_there_were != How_many_corn_seeds_there_were:
                ...
                t.sleep(1)
                if not mmod:
                    prco("\a获得成就[" + Fore.LIGHTGREEN_EX + "真的是小偷！" + Fore.RESET + "]")
                    mmod = True
                    t.sleep(1)
        else:
            Gold = 5
    ...
    # ↓死循环
    while True:
        if not figo and Gold > 5:
            prco("\a获得成就[" + Fore.LIGHTGREEN_EX + "第一桶金" + Fore.RESET + "]")
            figo = True
            t.sleep(1)

        elif neco and frsc and figo and deal and coon and cnsc and stol and cost and aoag is False:
            prco("\a获得成就[" + Fore.LIGHTYELLOW_EX + "我会玩了" + Fore.RESET + "]")
            aoag = True
            t.sleep(1)

        elif Number_of_thefts >= 50 and not prof:
            prco("\a获得成就[" + Fore.LIGHTYELLOW_EX + "专业小偷" + Fore.RESET + "]")
            prof = True
            t.sleep(1)

        elif Number_of_transactions >= 50 and not pref:
            prco("\a获得成就[" + Fore.LIGHTYELLOW_EX + "老顾客,有优惠吗?" + Fore.RESET + "]")
            pref = True
            t.sleep(1)

        elif not pogo and Gold >= 50:
            prco("\a获得成就[" + Fore.LIGHTYELLOW_EX + "真的是一桶金" + Fore.RESET + "]")
            pogo = True
            t.sleep(1)

        elif Number_of_thefts >= 100 and not moex:
            prco("\a获得成就[" + Fore.LIGHTMAGENTA_EX + "小偷院长" + Fore.RESET + "]")
            moex = True
            t.sleep(1)

        elif not golu and Number_of_not_bullied >= 10:
            prco("\a获得成就[" + Fore.LIGHTMAGENTA_EX + "欧皇圣体" + Fore.RESET + "]")
            golu = True
            t.sleep(1)

        elif not balu and Number_of_been_bullied >= 10:
            prco("\a获得成就[" + Fore.LIGHTMAGENTA_EX + "天生非酋体质" + Fore.RESET + "]")
            balu = True
            t.sleep(1)

        elif not ripe and Gold >= 500:
            prco("\a获得成就[" + Fore.LIGHTMAGENTA_EX + "大富翁" + Fore.RESET + "]")
            ripe = True
            t.sleep(1)
        ...
        if Whether_or_not_to_steal_it == "T" and How_much_feed_the_chicks_ate_in_total >= 20:
            Number_of_stolen += 1
            Manor_Lord = r.choice(Manor_Lord)
        ...
        if What_to_do == "1":
            ...
            if Whether_to_use_an_accelerator_card == "1" and Whether_to_use_stealth_potions == "2":
                ...
                if not coon:
                    prco("\a获得成就[" + Fore.LIGHTGREEN_EX + "撸起袖子加油吃" + Fore.RESET + "]")
                    coon = True
            if Whether_to_use_an_accelerator_card == "2" and Whether_to_use_stealth_potions == "1":
            ...
            if pope:
                pope_status = "[" + Fore.LIGHTYELLOW_EX + "礼貌问候" + Fore.LIGHTWHITE_EX + "]" + Fore.RESET
            else:
                pope_status = "[????????]"
...
{soha_status}━━━━╋━━━━━{moex_status}
                  ┗━━━━━{ripe_status}━━━━━{mmod_status}

{mmod_status}                                                    {pope_status}
                                               """
            ...
            if not pope:
                prco("你好，有什么事吗？")
                t.sleep(0.5)
                prco("\a获得成就[" + Fore.LIGHTGREEN_EX + "礼貌问候" + Fore.RESET + "]")
                pope = True
                t.sleep(0.5)
            else:
                prco("\a你好，有什么事吗？")
