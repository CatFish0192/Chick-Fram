...
def print_progress_bar(iteration, total, prefix="",
    ...
    print(f"\r{prefix} [{bar}] {percent}% {suffix}", end="\r") # 效果类似：描述文字 [=========================] 100.0%
...
{os.get_terminal_size().columns * "─"}\033[0m""" + f"""


{int((os.get_terminal_size().columns-4)/2) * " "}【引言】
...
{int((os.get_terminal_size().columns-30)/2) * " "}【关于复制、分发和修改的术语和条件】
〇、定义
...
{int((os.get_terminal_size().columns-32)/2) * " "}【附录：如何将上述条款应用到你的新程序】
...
它根据 GNU 通用公共许可证的条款发布Free Software Foundation，许可证的第 3 版，或（由您选择）任何更高版本。
    ...
    if not os.path.exists(f"XJZY-{version}.pkl"):
        #新手引导
    ...
    while True:
        ...
        if Whether_or_not_to_steal_it == "T" and feeds >= 20:
            stolen_time += 1
            Manor_Lord_2 = random.choice(Manor_Lord_1)
...
{soha_status}━━━━╋━━━━━{moex_status}
            ...
            if moreoptions == "1":
                ...
                if os.name == "nt":
                    os.system("start https://ejkdn.github.io/XJZY-Wiki/")
                    prco("已打开浏览器", 4)
                else:
            ...
            showmenu("debugmenu")
            opendebug = input("\033[1;34m请选择开启项目：\033[0m")
            if opendebug == "1":
                savedate = True
                prco("已开启数据保存功能", 4)
                time.sleep(1)
                ...
                if deletedate == "1":
                    ...
                    write_data(version, data_pack)
                    savedate = False
                    if os.path.exists(f"XJZY-{version}.pkl"):
                        os.remove(f"XJZY-{version}.pkl")
                        prco("\a删除完成，请重启游戏。", 4)
                    else:
                        prco("找不到数据库文件", 1)
...
if __name__ == "__main__":
    XJZY("2.9Beta9", "重写颜色函数和代码优化！") #调用主函数
