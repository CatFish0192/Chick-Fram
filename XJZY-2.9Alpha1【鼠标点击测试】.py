...
# 打包代码（小白专用）：pyinstaller --paths D:\code_storehouse\Python3.11.5\Lib\site-packages\colorama -F XJZY.py -i D:\code\Python\XJZY-V2\XJZY-2.9(Test)\ico\1.ico
...
def wjf(wdate=None):
    ...
    if wdate is None:
        date = {
                    ...
                    "accomplishment":{
                        # 普通
                        "neco":False, "frsc":False, "figo":False,
                        "deal":False, "coon":False, "cnsc":False,
                        # 高级
                        "aoag":False, "stol":False, "cost":False,
                        "prof":False, "pope":False, "pogo":False,
                        "pref":False,
                        # 挑战
                        "soha":False, "moex":False, "golu":False,
                        "balu":False, "eate":False, "mmod":False,
                        "ripe":False
                    },
                    "accomplishment_date":{
                        "Number_of_thefts":0,
        ...
        with open(datepath, "w+", encoding="UTF-8") as json_file:
...
def rjf():
    ...
    with open(datepath, "r", encoding="UTF-8") as json_file:
...
def logo(status="start"):
    ...
    if status == "start": # 开始动画
        ...
        endlogo_x = int((width - startlogo_width) / 2) # 计算出logo的准确位置，并把不能整除的转换为整数
        endlogo_y = int((height - startlogo_height) / 2)
...
def check_achievements():
    ...
    global Number_of_thefts
    global Number_of_transactions
    global Number_of_stolen
    global Number_of_been_bullied
    global Number_of_not_bullied
...
def XJZY(): #主函数
    from colorama import init, Fore  # ←导入颜色模块(依赖)
...
它根据 GNU 通用公共许可证的条款发布Free Software Foundation，许可证的第 3 版，或（由您选择）任何更高版本。
    ...
    10:2, 11:2.1, 12:2.2, 13:2.3, 14:2.4,
    15:2.5, 16:2.6, 17:2.7, 18:2.8, 19:2.9,
    20:3, 21:3.1, 22:3.2, 23:3.3, 24:3.4,
    ...
    if not o.path.exists(f"XJZY-{VERSION}.db"):
        ...
        wjf()
        #初始化
        conn = sqlite3.connect(f"XJZY-{VERSION}.db") #创建数据库文件
        cursor = conn.cursor() #创建游标
        cursor.execute("create table xjzy (id int(1) primary key, golds float, stealth_potions int, feeds int, corn_feeds int, accelerator_cards int, corns int, corn_needs int, chick_level int)") #数据库结构
        cursor.execute("insert into xjzy (id, golds, stealth_potions, feeds, corn_feeds, accelerator_cards, corns, corn_needs, chick_level) values (1, 0, 0, 0, 0, 0, 0, 0, 1)") #给数据库赋值
        ...
        prco(f"版本：{VERSION}")
        prco("公告：小鸡把图标画出来了？")
        conn = sqlite3.connect(f"XJZY-{VERSION}.db") #选择数据库文件
        cursor = conn.cursor()
        ...
        if o.path.exists(datepath):
            date = rjf()
            # json数据
            json_golds = date["date"]["golds"]
            json_stealth_potions = date["date"]["stealth_potions"]
            json_feeds = date["date"]["feeds"]
            json_corn_feeds = date["date"]["corn_feeds"]
            json_accelerator_cards = date["date"]["accelerator_cards"]
            json_corns = date["date"]["corns"]
            json_corn_needs = date["date"]["corn_needs"]
            json_chick_level = date["date"]["chick_level"]
            json_eating_speed = date["date"]["eating_speed"]
            ...
            json_golds = float(json_golds)
            json_stealth_potions = int(json_stealth_potions)
            json_feeds = int(json_feeds)
            json_corn_feeds = int(json_corn_feeds)
            json_accelerator_cards = int(json_accelerator_cards)
            json_corns = int(json_corns)
            json_corn_needs = int(json_corn_needs)
            eating_speed = int(json_eating_speed)
        else:
            # xjzydbresult[0]是id字段
            golds = xjzydbresult[1]
            stealth_potions = xjzydbresult[2]
            feeds = xjzydbresult[3]
            corn_feeds = xjzydbresult[4]
            accelerator_cards = xjzydbresult[5]
            corns = xjzydbresult[6]
            corn_needs = xjzydbresult[7]
            chick_level = xjzydbresult[8]

            json_golds = golds
            json_stealth_potions = stealth_potions
            json_feeds = feeds
            json_corn_feeds = corn_feeds
            json_accelerator_cards = accelerator_cards
            json_corns = corns
            json_corn_needs = corn_needs
            json_chick_level = chick_level
        ...
        if xjzydbresult != None: #数据库为空
            ...
            if json_golds != golds or json_stealth_potions != stealth_potions or json_feeds != feeds or json_accelerator_cards != accelerator_cards or json_corns != corns or json_corn_needs != corn_needs:
                golds = json_golds
                stealth_potions = json_stealth_potions
                feeds = json_feeds
                corn_feeds = json_corn_feeds
                accelerator_cards = json_accelerator_cards
                corns = json_corns
                corn_needs = json_corn_needs
                chick_level = json_chick_level

                prco("不要再偷鸡了，这样不会有好下场的-.-",1)
                t.sleep(1)
                if not mmod:
                    prco("\a获得成就[" + Fore.LIGHTMAGENTA_EX + "真的是小偷！" + Fore.RESET + "]")
                    mmod = True
                    t.sleep(1)
        else:
            golds = 5
    ...
    # ↓死循环
    while True:
        eating_speed = Get_speed[chick_level-1]
        # 判断是否获得成就
        ...
        if savedate:
            wdate = {
                        ...
                        "accomplishment":{
                            # 普通
                            "neco":neco, "frsc":frsc, "figo":figo,
                            "deal":deal, "coon":coon, "cnsc":cnsc,
                            # 高级
                            "aoag":aoag, "stol":stol, "cost":cost,
                            "prof":prof, "pope":pope, "pogo":pogo,
                            "pref":pref,
                            # 挑战
                            "soha":soha, "moex":moex, "golu":golu,
                            "balu":balu, "eate":eate, "mmod":mmod,
                            "ripe":ripe,
                        },
                        "accomplishment_date":{
            ...
            # ↓保存数据
            cursor.execute("update xjzy set golds = ?, stealth_potions = ?, feeds = ?, corn_feeds = ?, accelerator_cards = ?, corns = ?, corn_needs = ? where id = 1", (golds, stealth_potions, feeds, corn_feeds, accelerator_cards, corns, corn_needs))
            conn.commit() #提交事务
        ...
        if Whether_or_not_to_steal_it == "T" and feeds >= 20:
            ...
            if not stol:
                ...
                stol = True
                t.sleep(1)
            elif not eate and Number_of_stolen >= 10:
                ...
                if Why == "1":
                    ...
                    try:
                        ...
                        if int(golds) < 5:
                            ...
                            if corns == 0:
                                prco("你现在没有玉米哦！", 1)
                        elif int(How_many_acceleration_corn_feeds_to_process) < 0:
                            prco("老板把你赶了出去", 1)
...
{soha_status}━━━━╋━━━━━{moex_status}
                ...
                if deletedate == "1":
                    if o.name == "nt":
                        ...
                        if o.path.exists(f"XJZY-{VERSION}.db"):
                            cursor.execute("update xjzy set golds = ?, stealth_potions = ?, feeds = ?, accelerator_cards = ?, corns = ?, corn_needs = ? where id = 1", (golds, stealth_potions, feeds, accelerator_cards, corns, corn_needs)) #保存数据
                            cursor.close() #关闭游标
                            conn.commit() #提交事务
                            conn.close() #关闭数据库
                            savedate = False
            ...
            if savedate:
                ...
                wjf(wdate)
                cursor.execute("update xjzy set golds = ?, stealth_potions = ?, feeds = ?, corn_feeds = ?, accelerator_cards = ?, corns = ?, corn_needs = ?, chick_level = ? where id = 1", (golds, stealth_potions, feeds, corn_feeds, accelerator_cards, corns, corn_needs, chick_level)) #保存数据
                cursor.close() #关闭游标
                conn.commit() #提交事务
                conn.close() #关闭数据库
