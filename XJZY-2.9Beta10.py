...
"""
# 打包代码：pyinstaller -F <小鸡庄园文件名>.py -i <图标目录>
# 打包代码（白猫专用）：pyinstaller -F XJZY.py -i D:\code\Python\XJZY-V2\XJZY-2.9(Test)\ico\玉米.ico
from re import findall  # ←导入正则表达式匹配库中的findall函数
from pickle import dump, load  # ←导入pickle加密库
import keyboard # ←导入鼠标检测模块
import random  # ←导入随机数模块
import os  # ←导入系统模块

os.environ['PYGAME_HIDE_SUPPORT_PROMPT'] = "hide" #关闭pygame版本信息显示

import pygame  # ←pygame用来播放阴乐
import time  # ←导入时间模块
import sys # ←导入系统功能模块
...
try:
    ...
    print("\033[1;31m无法获取终端大小，请使用系统自带终端运行！\033[0m")
    input()
    sys.exit()

def prco(text, color=3):
    color_list = {
    31:"\033[1;31m" + text + "\033[0m",
    32:"\033[1;32m" + text + "\033[0m",
    33:"\033[1;33m" + text + "\033[0m",
    34:"\033[1;34m" + text + "\033[0m",
    35:"\033[1;35m" + text + "\033[0m",
    36:"\033[1;36m" + text + "\033[0m",
    37:"\033[1;37m" + text + "\033[0m"
    }
    if color == 1:
        print(color_list[31]) #red:坏、失败、错误
    elif color == 2:
        print(color_list[36]) #cyan:菜单
    elif color == 3:
        print(color_list[37]) #white:信息
    elif color == 4:
        print(color_list[32]) #green:好、成功
    elif color == 5:
        print(color_list[33]) #yellow:提示
    elif color == 6:
        print(color_list[34]) #blue:选择
    elif color == 7:
        print(color_list[35]) #magenta:挑战

def print_progress_bar(iteration, total, prefix="",
                       suffix="", decimals=1, length=columns-18,
                       fill="="):
    """
    进度条（感谢国外佚名大佬提供）
    """
    ...
    bar = ("\033[1;32m" + fill + "\033[0m") * filledLength + ("\033[1;37m" + fill + "\033[0m") * (length - filledLength)
...
def logo(version, status="start"):
    ...
    VERSION_LEN = len(version) # ←版本名长度
    width = columns # 获取终端的长度
    height = os.get_terminal_size().lines # 获取终端的宽度

    clear()
    if not os.path.exists(f"XJZY-{version}.pkl"): # 第一次启动时的logo
        # 开始logo的尺寸
        startlogo_width = 54 # logo的最大长度
        startlogo_height = 8 # logo的最大宽度

        # 开始logo
        startlogo = {
        "1":f"{(int((startlogo_width-26)/2))*' '}   _  __    _________  __",
        "2":f"{(int((startlogo_width-26)/2))*' '}  | |/ /   / /__  /\ \/ /",
        "3":f"{(int((startlogo_width-26)/2))*' '}  |   /_  / /  / /  \  /",
        "4":f"{(int((startlogo_width-26)/2))*' '} /   / /_/ /  / /__ / /",
        "5":f"{(int((startlogo_width-26)/2))*' '}/_/|_\____/  /____//_/",
        "6":f"{(int((startlogo_width-VERSION_LEN-3)/2))*' '}{version}", # 版本号居中（也许有点乱）
        "7":"小鸡庄园是一款文字游戏，由“XJZY开发组”开发和测试",
        "8":f"{(int((startlogo_width-22)/2))*' '}版权所有 (C) 2025 小白"
        }
    else: # 之后的logo
        ...
        "4":" /   / /_/ /  / /__ / /",
        "5":"/_/|_\____/  /____//_/",
        "6":f"{(int((startlogo_width-VERSION_LEN-3)/2))*' '}{version}{(int((startlogo_width-VERSION_LEN-3)/2))*' '}"
        }
...
def showmenu(status="initialmenu"):
    if type((columns-10)/2) == int:
        initialmenu = f"""
╭{(columns-2)*"─"}╮
│{((columns-10)/2)*" "}小鸡庄园{((columns-10)/2)*" "}│
├{(columns-2)*"─"}┤
│1 偷吃{(columns-8)*" "}│
│2 种玉米{(columns-10)*" "}│
│3 镇商店{(columns-10)*" "}│
...
╰{(columns-2)*"─"}╯
        """
    else: # 如果屏幕（终端）宽度很刁钻，左边会多出来一个空格
        initialmenu = f"""
╭{(columns-2)*"─"}╮
│{int((columns-10)/2)*" "}小鸡庄园{int((columns-9)/2)*" "}│
├{(columns-2)*"─"}┤
│1 偷吃{(columns-8)*" "}│
│2 种玉米{(columns-10)*" "}│
│3 镇商店{(columns-10)*" "}│
...
╰{(columns-2)*"─"}╯
        """
    if type((columns-8)/2) == int:
        shopmenu = f"""
╭{(columns-2)*"─"}╮
│{((columns-8)/2)*" "}商店{((columns-8)/2)*" "}│
├{(columns-2)*"─"}┤
│1 卖饲料{(columns-10)*" "}│
│2 买加速卡{(columns-12)*" "}│
│3 买隐身药剂{(columns-14)*" "}│
...
╰{(columns-2)*"─"}╯
        ...
        shopmenu = f"""
╭{(columns-2)*"─"}╮
│{int((columns-6)/2)*" "}商店{int((columns-5)/2)*" "}│
├{(columns-2)*"─"}┤
│1 卖饲料{(columns-10)*" "}│
│2 买加速卡{(columns-12)*" "}│
│3 买隐身药剂{(columns-14)*" "}│
...
╰{(columns-2)*"─"}╯
    ...
    if type((columns-8)/2) == int:
        moremenu = f"""
╭{(columns-2)*"─"}╮
│{((columns-8)/2)*" "}更多{((columns-8)/2)*" "}│
├{(columns-2)*"─"}┤
│1 关于{(columns-8)*" "}│
│2 Wiki{(columns-8)*" "}│
│3 源代码{(columns-10)*" "}│
│4 GPLv3许可证{(columns-15)*" "}│
├{(columns-2)*"─"}┤
│0 返回{(columns-8)*" "}│
╰{(columns-2)*"─"}╯
        """
        ...
        moremenu = f"""
╭{(columns-2)*"─"}╮
│{int((columns-6)/2)*" "}更多{int((columns-5)/2)*" "}│
├{(columns-2)*"─"}┤
│1 关于{(columns-8)*" "}│
│2 Wiki{(columns-8)*" "}│
│3 源代码{(columns-10)*" "}│
...
╰{(columns-2)*"─"}╯
        ...
        debugmenu = f"""
╭{(columns-2)*"─"}╮
│{int((columns-7)/2)*" "}Debug{int((columns-6)/2)*" "}│
├{(columns-2)*"─"}┤
│1 开启数据保存{(columns-16)*" "}│
│2 关闭数据保存{(columns-16)*" "}│
│3 清除数据{(columns-12)*" "}│
...
╰{(columns-2)*"─"}╯
        """
    if type((columns-18)/3) == int:
        selectivemenu1 = f"""
╭{(columns-2)*"─"}╮
│{((columns-18)/3)*" "}1 确定{((columns-18)/3)*" "}2 取消{int((columns-5)/3)*" "}│
╰{(columns-2)*"─"}╯
        """
    else:
        selectivemenu1 = f"""
╭{(columns-2)*"─"}╮
│{int((columns-16)/3)*" "}1 确定{int((columns-18)/3)*" "}2 取消{int((columns-5)/3)*" "}│
╰{(columns-2)*"─"}╯
        """
    if type((columns-18)/3) == int:
        warningmenu = f"""
...
╰{(columns-2)*"─"}╯
        """
    if type((columns-16)/3) == int:
        selectivemenu2 = f"""
╭{(columns-2)*"─"}╮
│{((columns-16)/3)*" "}1 要{((columns-16)/3)*" "}2 不要{int((columns-5)/3)*" "}│
╰{(columns-2)*"─"}╯
        """
    else:
        selectivemenu2 = f"""
╭{(columns-2)*"─"}╮
│{int((columns-14)/3)*" "}1 要{int((columns-16)/3)*" "}2 不要{int((columns-5)/3)*" "}│
╰{(columns-2)*"─"}╯
        """
...
def check_achievements():
    ...
    if neco and frsc and figo and deal and coon and cnsc and stol and cost and aoag is False:
        achievements.append("\033[1;33m我会玩了\033[0m")
        aoag = True

    if stol and cost and aoag and prof and pope and pogo and pref and soha is False:
        achievements.append("\033[1;33m来点更难的？\033[0m")
        soha = True

    if thefts_time >= 50 and not prof:
        achievements.append("\033[1;33m专业小偷\033[0m")
        prof = True

    if transactions_time >= 50 and not pref:
        achievements.append("\033[1;33m老顾客,有优惠吗?\033[0m")
        pref = True

    if thefts_time >= 100 and not moex:
        achievements.append("\033[1;35m小偷院长\033[0m")
        moex = True

    if not golu and not_bullied_time >= 10:
        achievements.append("\033[1;35m欧皇圣体\033[0m")
        golu = True

    if not balu and bullied_time >= 10:
        achievements.append("\033[1;35m天生非酋体质\033[0m")
        balu = True

    if not ripe and golds >= 500:
        achievements.append("\033[1;35m大富翁\033[0m")
        ripe = True
...
def about(version):
    ...
    height = os.get_terminal_size().lines # 获取终端的宽度
    # 尺寸
    about_height = 20 # about信息的宽度

    # about信息
    about = {
    "1":f"{(int((columns-25)/2))*' '}   _  __    _________  __",
    "2":f"{(int((columns-25)/2))*' '}  | |/ /   / /__  /\ \/ /",
    "3":f"{(int((columns-25)/2))*' '}  |   /_  / /  / /  \  /",
    "4":f"{(int((columns-25)/2))*' '} /   / /_/ /  / /__ / /",
    "5":f"{(int((columns-25)/2))*' '}/_/|_\____/  /____//_/",
    "6":f"{(int((columns-VERSION_LEN)/2))*' '}{version}", # 版本号居中
    "7":"",
    "8":f"{int(((columns-4)/2))*' '}鸣谢",
    "9":f"{int(((columns-4)/2))*' '}白猫",
    "10":f"{int(((columns-4)/2))*' '}橘猫",
    "11":f"{int(((columns-5)/2))*' '}old唐",
    "12":"",
    "13":f"{int(((columns-4)/2))*' '}更多",
    "14":f"{int(((columns-27)/2))*' '}https://gitee.com/PSWG/XJZY",
    "15":f"{int(((columns-33)/2))*' '}https://gitcode.com/bxl24563/XJZY",
    "16":"",
    "17":f"{int(((columns-48)/2))*' '}小鸡庄园是一款文字游戏，由“XJZY开发组”开发和测试",
    "18":f"{int(((columns-22)/2))*' '}版权所有 (C) 2025 白猫",
    "19":f"{int(((columns-43)/2))*' '}本程序是自由软件：您可以重新分发和/或修改它",
    "20":f"{int(((columns-104)/2))*' '}它根据 GNU 通用公共许可证的条款发布Free Software Foundation，许可证的第 3 版，或（由您选择）任何更高版本"
    }

    line = 1
    about_y = int((height - about_height) / 2)
    for a in range(about_y): # 上面的空行
        print()

    for b in range(1):
        for d in range(about_height): #完善的一部分about信息
            print(about[str(line)])
            int(line)

            if line < about_height:
                line += 1 #继续下一部分about信息
            else:
                break # 完成
...
def XJZY(version, announcement): #主函数。参数一是版本号，参数二是公告
    ...
    copyright = f"""\033[1;31m
{columns * "─"}
这是 GNU 通用公共许可证（GNU GPL）的非正式简体中文翻译
它并未被自由软件基金会发行，也不是使用 GNU GPL 软件的发布法律声明——只有 GNU GPL 的原始英文版(https://www.gnu.org/licenses/gpl-3.0.zh-cn.html)才有法律意义
...
{int((columns-30)/2) * " "}【关于复制、分发和修改的术语和条件】
〇、定义
...
{int((columns-32)/2) * " "}【附录：如何将上述条款应用到你的新程序】
...
　　本GNU通用公共许可协议不允许把你的程序并入私有程序。如果你的程序是某种库，且你想允许它被私有程序链接而使之更有用，请使用GNU较宽松通用公共许可协议。决定前请先查阅<http://www.gnu.org/philosophy/why-not-lgpl.html>。
    ...
    if not os.path.exists(data_name):
        ...
        if data_pack != None: #数据不为空
            try:
                ...
                time.sleep(1)
                if not mmod:
                    prco("获得成就[\033[1;35m真的是小偷！\033[0m]")
                    mmod = True
                    time.sleep(1)
                golds = 5
                stealth_potions = 0
    ...
    while True:
        ...
        Whether_or_not_to_steal_it = random.choice(Stealing_probability)
        # 判断是否获得成就
        ...
        if Whether_or_not_to_steal_it == "T" and feeds >= 20:
            stolen_time += 1
            Choose_Manor_Lord = random.choice(Manor_Lord)
            How_many_minutes_were_eaten = random.randint(15, 30)
            ...
            if not stol:
                prco("\a获得成就[\033[1;33m黑吃黑？\033[0m]")
                stol = True
                time.sleep(1)
            elif not eate and stolen_time >= 10:
                prco("\a获得成就[\033[1;35m黑吃黑,我是被吃的\033[0m]")
                eate = True
                time.sleep(1)
            else:
                time.sleep(1)
        ...
        What_to_do = input("\033[1;34m你要干什么呢？\033[0m")

        if What_to_do == "1":
            if accelerator_cards > 0:
                showmenu("selectivemenu2")
                time.sleep(0.5)
                Whether_to_use_an_accelerator_cards = input("\033[1;34m你要给小鸡使用加速卡吗？\033[0m")
            else:
            ...
            if stealth_potions > 0:
                showmenu("selectivemenu2")
                time.sleep(0.5)
                Whether_to_use_stealth_potions = input("\033[1;34m你要给小鸡使用隐身药剂吗？\033[0m")
            else:
            ...
            if Whether_to_use_an_accelerator_cards == "1" and Whether_to_use_stealth_potions == "1":
                ...
                for i in range(100):
                    print_progress_bar(i + 1, 100, prefix="偷吃中:")
                    time.sleep(How_many_minutes_were_eaten/1000)
                if not frsc:
                    prco("\a获得成就[\033[1;32m白手起家\033[0m]")
                    frsc = True
                    time.sleep(1)
                prco(
                    "你的小鸡在" +
                ...
                if not coon:
                    prco("\a获得成就[\033[1;32m撸起袖子加油吃\033[0m]")
                    coon = True

                if not cnsc:
                    prco("\a获得成就[\033[1;32m看,看不清楚\033[0m]")
                    cnsc = True
            ...
            if Whether_to_use_an_accelerator_cards == "2" and Whether_to_use_stealth_potions == "2":
                ...
                Whether_to_be_beaten_or_not = random.choice(Whether_to_be_beaten_or_not)
                if Whether_to_be_beaten_or_not == "F":
                    ...
                    if plead == "F":
                        ...
                        prco("饲料-10", 1)
                    if not cost:
                        prco("\a获得成就[\033[1;33m什么都是有代价的\033[0m]")
                        cost = True
            ...
            if corn_needs == 0:
                ...
                try:
                    How_much_to_plant = int(input("\033[1;34m你要种多少玉米？\033[0m"))
                except ValueError:
                    prco("额，看来你不想种玉米", 1)
                else:
                    if How_much_to_plant > corn_needs != 0:
                        ...
                        if How_much_to_plant < 10:
                            Corn_that_grows = random.randint(0, How_much_to_plant)
                        else:
                            Corn_that_grows = random.randint(How_much_to_plant-10, How_much_to_plant)
                        corns += Corn_that_grows

                        for i in range(100):
                            print_progress_bar(i + 1, 100, prefix="播种中:")
                            time.sleep(How_much_to_plant/1000)
            ...
            while True:
                if not figo and golds > 5:
                    prco("\a获得成就[\033[1;32m第一桶金\033[0m]")
                    figo = True
                    time.sleep(0.5)

                if not pogo and golds >= 50:
                    prco("\a获得成就[\033[1;33m真的是一桶金\033[0m]")
                    pogo = True

                showmenu("shopmenu")
                Why = input("\033[1;34m你要做什么买卖？\033[0m")
                if Why == "1":
                    prco("5克饲料=1个金币", 5)
                    prco("你有" + str(feeds) + "克饲料", 5)
                    try:
                        How_much_feed_to_buy = int(input("\033[1;34m你要卖多少饲料？\033[0m"))
                    except ValueError:
                    ...
                    try:
                        How_many_acceleration_cards_to_buy = int(input("\033[1;34m你要买多少加速卡：\033[0m"))
                    except ValueError:
                        prco("老板把你赶了出去", 1)
                    else:
                    ...
                    prco("1瓶隐身药剂=15个金币", 5)
                    try:
                        How_much_stealth_potion_to_buy = int(input("\033[1;34m你要买多少隐身药剂？\033[0m"))
                    except ValueError:
                        prco("老板把你赶了出去", 1)
                    else:
                    ...
                    try:
                        How_many_acceleration_corn_feeds_to_process = int(input("\033[1;34m你要加工多少份玉米饲料：\033[0m"))
                    except ValueError:
                        prco("老板把你赶了出去", 1)
                    else:
                    ...
                    prco("1个玉米种子=5个金币", 5)
                    try:
                        How_many_corn_seeds_to_buy = int(input("\033[1;34m你要买多少玉米种子？\033[0m"))
                    except ValueError:
                        prco("老板把你赶了出去", 1)
                    else:
                    ...
                    prco("1个玉米=10个金币", 5)
                    try:
                        How_much_corn = int(input("\033[1;34m你要卖多少玉米？\033[0m"))
                    except ValueError:
                        prco("客人不知道你要干嘛，你什么都没卖出去", 1)
                    else:
                        if int(How_much_corn) < 0:
                            ...
                            if not deal:
                                prco("\a获得成就[\033[1;32m成交！\033[1;32m]")
                                deal = True
                                time.sleep(1)
                            transactions_time += 1
                elif Why == "0":
            ...
            # 普通
            if neco:
                neco_status = "[\033[1;32m初来乍到\033[1;37m]\033[0m"
            else:
                neco_status = "[????????]"

            if frsc:
                frsc_status = "[\033[1;32m白手起家\033[1;37m]\033[0m"
            else:
                frsc_status = "[????????]"

            if figo:
                figo_status = "[\033[1;32m第一桶金\033[1;37m]\033[0m"
            else:
                figo_status = "[????????]"

            if deal:
                deal_status = "[\033[1;32m成交！\033[1;37m]\033[0m"
            else:
                deal_status = "[??????]"

            if coon:
                coon_status = "[\033[1;32m撸起袖子加油吃\033[1;37m]\033[0m"
            else:
                coon_status = "[??????????????]"

            if cnsc:
                cnsc_status = "[\033[1;32m看，看不清楚\033[1;37m]\033[0m"
            else:
                cnsc_status = "[????????????]"
            # 高级
            if aoag:
                aoag_status = "[\033[1;33m我会玩了\033[1;37m]\033[0m"
            else:
                aoag_status = "[????????]"

            if cost:
                cost_status = "[\033[1;33m什么都是有代价的\033[1;37m]\033[0m"
            else:
                cost_status = "[????????????????]"

            if stol:
                stol_status = "[\033[1;33m黑吃黑？\033[1;37m]\033[0m"
            else:
                stol_status = "[????????]"

            if prof:
                prof_status = "[\033[1;33m专业小偷\033[1;37m]\033[0m"
            else:
                prof_status = "[????????]"

            if pope:
                pope_status = "[\033[1;35m礼貌问候\033[1;37m]\033[0m"
            else:
                pope_status = "[????????]"

            if pogo:
                pogo_status = "[\033[1;33m真的是一桶金\033[1;37m]\033[0m"
            else:
                pogo_status = "[????????????]"

            if pref:
                pref_status = "[\033[1;33m老顾客，有优惠吗？\033[1;37m]\033[0m"
            else:
                pref_status = "[??????????????????]"
            # 挑战
            if soha:
                soha_status = "[\033[1;35m来点更难的？\033[1;37m]\033[0m"
            else:
                soha_status = "[????????????]"
            if moex:
                moex_status = "[\033[1;35m小偷院长\033[1;37m]\033[0m"
            else:
                moex_status = "[????????]"

            if golu:
                golu_status = "[\033[1;35m欧皇圣体\033[1;37m]\033[0m"
            else:
                golu_status = "[????????]"

            if balu:
                balu_status = "[\033[1;35m天生非酋体质\033[1;37m]\033[0m"
            else:
                balu_status = "[????????????]"

            if eate:
                eate_status = "[\033[1;35m黑吃黑，我是被吃的\033[1;37m]\033[0m"
            else:
                eate_status = "[??????????????????]"

            if mmod:
                mmod_status = "[\033[1;35m真的是小偷！\033[1;37m]\033[0m"
            else:
                mmod_status = "[??????????]"

            if ripe:
                ripe_status = "[\033[1;35m大富翁\033[1;37m]\033[0m"
            else:
                ripe_status = "[??????]"
...
{soha_status}━━━━╋━━━━━{moex_status}
            ...
            if chick_level == 30:
                ...
                print(str(chick_level) + "LV ──> \033[1;32m" + str(chick_level+1) + "LV\n\033[0m")
                prco("这次升级需要" + str((chick_level+1) * 5) + "克玉米饲料", 5)
                if corn_feeds != 0:
                    prco("你现在有" + str(corn_feeds) + "克玉米饲料", 5)
                    showmenu("selectivemenu1")
                    Whether_to_upgrade = input("\033[1;34m确定要升级吗？\033[0m")
                    if Whether_to_upgrade == "1":
                        if corn_feeds >= chick_level * 5:
                            for i in range(100):
                                print_progress_bar(i + 1, 100, prefix="升级中:")
                                time.sleep((chick_level+1)/100)
            ...
            showmenu("moremenu")
            moreoptions = input("\033[1;34m请选择：\033[0m")
            if moreoptions == "1":
                about(version)
            elif moreoptions == "2":
                if os.name == "nt":
                    os.system("start https://xjzy-wiki.github.io/")
                    prco("已打开浏览器", 4)
                else:
                    prco("把这个链接复制到浏览器打开：https://xjzy-wiki.github.io/")
            elif moreoptions == "3":
                if os.name == "nt":
                    os.system("start https://gitee.com/PSWG/XJZY/")
                    prco("已打开浏览器", 4)
                else:
            ...
            showmenu("debugmenu")
            opendebug = input("\033[1;34m请选择项目：\033[0m")
            if opendebug == "1":
                savedate = True
                prco("已开启数据保存功能", 4)
                time.sleep(1)
                ...
                showmenu("warningmenu")
                cleardate = input("\033[1;31m确定要清除数据吗？\033[0m")
                if cleardate == "1":
                    golds = 5
                    stealth_potions = 0
                    feeds = 10
                ...
                showmenu("warningmenu")
                deletedate = input("\033[1;31m确定要删除数据吗？\033[0m")
                if deletedate == "1":
                    data_pack = {
                        "golds":golds,
                        "stealth_potions":stealth_potions,
            ...
            if not pope:
                prco("你好，有什么事吗？")
                time.sleep(0.5)
                prco("\a获得成就[\033[1;35m礼貌问候\033[0m]")
                pope = True
                time.sleep(0.5)
            else:
                prco("\a你好，有什么事吗？")
